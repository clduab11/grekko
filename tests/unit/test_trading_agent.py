...
class TestTradingAgent:
    ...
    def mock_config(self):
        ...
        return {
            "llm": {"model_name": "gpt-4", "temperature": 0.1, "timeout": 30},
            "risk": {
                "profile": "moderate",
        ...
        mock_response.choices[0].message.content = json.dumps(
            {
                ...
                "stop_loss": 42000,
                "reasoning": (
                    "Strong bullish signals with increasing volume and "
                    "positive sentiment."
                ),
            }
        )
    ...
    def trading_agent(self, mock_config, mock_openai_client):
        ...
        with patch(
            "src.ai_adaptation.agent.trading_agent.TradingAgent._load_config",
            return_value=mock_config,
        ):
            with patch.dict(os.environ, {"OPENAI_API_KEY": "mock_api_key"}):
                # Mock dependent components
                with patch(
                    "src.ai_adaptation.agent.trading_agent.StrategyManager"
                ):
                    with patch(
                        "src.ai_adaptation.agent.trading_agent.RiskManager"
                    ):
                        with patch(
                            "src.ai_adaptation.agent.trading_agent."
                            "ExecutionManager"
                        ):
                            with patch(
                                "src.ai_adaptation.agent.trading_agent."
                                ...
                                agent.risk_manager.get_available_capital = (
                                    MagicMock(return_value=100000.0)
                                )
                                agent._get_current_price = MagicMock(
                                    return_value=43250.0
                                )
                                agent._get_portfolio_value = MagicMock(
    ...
    async def test_start_already_active(self, trading_agent):
        ...
        with patch.object(trading_agent, "_trading_loop", AsyncMock()):
            ...
            # Try to start again
            await trading_agent.start()
            # Should still be active and log a warning
            # (would check logs in a real test)
            assert trading_agent.is_active is True
    ...
    async def test_trading_loop_error_handling(self, trading_agent):
        ...
        trading_agent._gather_market_data = AsyncMock(
    ...
    async def test_gather_market_data(self, trading_agent):
        ...
        trading_agent._fetch_technical_analysis = AsyncMock(
            return_value={
                "indicators": {"rsi": 65},
                "price": {"current": 43250.0},
            }
        )
        trading_agent._fetch_additional_data = AsyncMock(
            return_value={
    ...
    async def test_make_trading_decisions(self, trading_agent):
        ...
        trading_agent._format_decision_prompt = MagicMock(
        ...
        trading_agent._format_decision_prompt.assert_called_once_with(
            "BTC/USDT", market_data["BTC/USDT"]
        )
    ...
    async def test_query_llm(self, trading_agent):
        ...
        assert (
            result
            == (
                trading_agent.client.chat.completions.create.return_value
            ...
            messages=[
                {
                    "role": "system",
                    "content": (
                        "You are Grekko, an advanced cryptocurrency "
                        "trading AI that provides reasoning in JSON format."
                    ),
                },
    ...
    def test_apply_risk_rules(self, trading_agent):
        ...
        assert (
            filtered[0]["trading_pair"] == "BTC/USDT"
        )  # First decision passed
        assert (
            filtered[1]["trading_pair"] == "LINK/USDT"
        )  # Third decision passed but was modified
        assert (
            filtered[1]["size"] == 0.20
        )  # Size was capped at max position size
    ...
    async def test_execute_decision_buy(self, trading_agent):
        ...
        assert (
            trading_agent.current_positions["BTC/USDT"]["entry_price"]
            == 43250.0
        )
        assert trading_agent.current_positions["BTC/USDT"]["amount"] == 3000.0
        assert (
            trading_agent.current_positions["BTC/USDT"]["stop_loss"] == 42000
        )
        assert (
            trading_agent.current_positions["BTC/USDT"]["target_price"]
            == 44000
        )
    ...
    async def test_execute_decision_sell(self, trading_agent):
        ...
        # Check that the order was created
        trading_agent.execution_manager.create_order.assert_called_once_with(
            symbol="BTC/USDT", order_type="market", side="sell", amount=0.5
        )
    ...
    def test_is_critical_error(self, trading_agent):
        ...
        assert (
            trading_agent._is_critical_error(
                Exception("Authentication failed")
            )
            is True
        )
        assert (
            trading_agent._is_critical_error(Exception("Insufficient funds"))
            is True
        )
        assert (
            trading_agent._is_critical_error(Exception("Connection error"))
            is True
        )

        # Test non-critical errors
        assert (
            trading_agent._is_critical_error(Exception("Some minor error"))
            is False
        )
        assert (
            trading_agent._is_critical_error(Exception("Operation timed out"))
            is False
        )
